...
└── src/
    ...
    ├── screens/
    │   ├── auth/
    │   │   ├── LoginScreen.js          # Connexion email/MDP + lien OTP + lien oublié
    │   │   ├── RegisterScreen.js       # Inscription
    │   │   ├── OtpScreen.js            # Connexion sans mot de passe (code email)
    │   │   └── ForgotPasswordScreen.js # Réinitialisation MDP par OTP (3 étapes)
    │   └── main/
    ...
    │       └── ProfileScreen.js        # Nom, mot de passe, dark mode, déconnexion
    └── components/
...
| Mot de passe oublié — 3 étapes (email → code OTP → nouveau MDP) | ForgotPassword |
...
- **KeyboardAvoidingView** : le chat utilise `behavior: 'padding'` (iOS) / `'height'` (Android) avec `paddingBottom: 110` dans la zone de saisie.
