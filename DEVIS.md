...
L'application mobile QuoteKeeper est developpee avec React Native et Expo SDK 54. Elle reproduit toutes les fonctionnalites du portail web et se connecte au meme service FastAPI.

### Technologies

- React Native 0.81.5 avec Expo SDK 54
- React Navigation v6 (Stack + Bottom Tabs)
- AsyncStorage (persistance locale du jeton JWT et du theme)
- Context API : AuthContext (session), ThemeContext (dark mode)
- Hooks personnalises : useApi, useQuote, useFavorites

### Fonctionnalites

- inscription et connexion par email/mot de passe
- recuperation de la citation aleatoire avec filtres par categorie et auteur
- citation du jour
- traduction en francais (MyMemory via le backend)
- copie dans le presse-papier
- ajout et suppression de favoris
- notes personnelles sur les favoris
- liste des favoris paginee (8 par page)
- modification du nom et du mot de passe sur la page profil
- mode sombre / clair persiste
...
Application React Native (Expo Go)
          |
          | requetes HTTP / JSON (fetch)
          v
Service web FastAPI (meme backend que le portail web)
```

### Lancement

```bash
# Terminal 1 — Tunnel backend (si le telephone n'est pas sur le meme reseau)
npx localtunnel --port 8000
# Copier l'URL affichee dans mobile/src/constants/api.js

# Terminal 2 — Metro avec tunnel
cd mobile && npx expo start --tunnel
```

Voir `mobile/README.md` pour les instructions completes.

### Criteres satisfaits (labo 4)

| Critere | Detail |
| ------- | ------ |
| Technologie React Native + Expo Go | Expo SDK 54, teste sur appareil physique |
| Interface fonctionnelle | 3 onglets : Accueil, Favoris, Profil |
| Appels reseau (fetch) | 10 endpoints utilises avec jeton JWT |
